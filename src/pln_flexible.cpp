...
// Helper struct for the Newton-Raphson solver
struct Functor {
  double y, mu, sigma;
  
  // First derivative of the log-integrand
  double df(double e) const {
    return y * sigma - sigma * std::exp(mu + sigma * e) - e;
  }
  
  // Second derivative of the log-integrand
  double ddf(double e) const {
    return -sigma * sigma * std::exp(mu + sigma * e) - 1.0;
  }
};
...
Rcpp::NumericVector dpln_rcpp(const Rcpp::IntegerVector& y,
  ...
  for (int i = 0; i < n_obs; ++i) {
    ...
    if (current_sigma <= 0) {
      log_lik[i] = -std::numeric_limits<double>::infinity();
      continue;
    }
    
    // --- STEP 1 & 2: Find Mode and Scale ---
    Functor F = { (double)current_y, current_mu, current_sigma };
    double mode = 0.0; // Initial guess for mode
    
    // Simple Newton-Raphson solver (10 iterations is usually plenty)
    for (int j = 0; j < 10; ++j) {
      double f_prime = F.df(mode);
      double f_double_prime = F.ddf(mode);
      if (f_double_prime == 0) break; // Avoid division by zero
      mode -= f_prime / f_double_prime;
    }
    
    double ddf_at_mode = F.ddf(mode);
    if (ddf_at_mode >= 0) { // Should be negative
      log_lik[i] = -std::numeric_limits<double>::infinity();
      continue;
    }
    double scale = std::sqrt(-1.0 / ddf_at_mode);
    
    // --- STEP 3: Adapted Quadrature ---
    std::vector<double> log_summands(K);
    double max_log_summand = -std::numeric_limits<double>::infinity();
    ...
    for (int k = 0; k < K; ++k) {
      ...
      if (wk <= 0) {
        log_summands[k] = -std::numeric_limits<double>::infinity();
        continue;
      }
      
      // Adapted node location
      double adapted_e = mode + zk * scale;
      
      // Log of the function f(epsilon) = y*log(lambda) - lambda - e^2/2
      double log_f_val = current_y * (current_mu + current_sigma * adapted_e) - 
        std::exp(current_mu + current_sigma * adapted_e) - 
        (adapted_e * adapted_e / 2.0);
      
      // Log of the transformed function h(zk) used in the summation
      log_summands[k] = std::log(wk) + std::log(scale) + log_f_val + (zk * zk / 2.0);
      
      if (log_summands[k] > max_log_summand) {
        max_log_summand = log_summands[k];
      }
    }
    
    if (std::isinf(max_log_summand) && max_log_summand < 0) {
      log_lik[i] = -std::numeric_limits<double>::infinity();
      continue;
    }
    ...
    double sum_exp = 0.0;
    for (int k = 0; k < K; ++k) {
      sum_exp += std::exp(log_summands[k] - max_log_summand);
    }
    
    double log_integral = max_log_summand + std::log(sum_exp);
    
    // The log(1/sqrt(2pi)) term from the original f(e) was dropped for the root finder,
    // but it was implicitly part of the integral. We add it back here.
    log_integral -= 0.5 * std::log(2.0 * M_PI);
    
    // Subtract log(y!) to get the final log probability
    log_lik[i] = log_integral - R::lgammafn(current_y + 1);
  }
